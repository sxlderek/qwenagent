#!/usr/bin/env bash
# cc-video.sh - Batch convert video files to MP4 with hardware encoding priority
...
# Include getopts_long library (embedded from UrsaDK/getopts_long)
getopts_long() {
...
build_subtitle_map() {
    ...
    # Set subtitle codec via nameref
    # shellcheck disable=SC2034
    subtitle_codec_ref="-c:s mov_text"
}
...
process_files() {
    ...
    for input_file in "${unique_files[@]}"; do
        ...
        # Add encoder flags (split into array to avoid word-splitting issues)
        local encoder_flags_arr
        read -ra encoder_flags_arr <<< "$encoder_flags"
        ffmpeg_cmd+=("${encoder_flags_arr[@]}")
